function NetworkField(t0, x, neq, nNeurons, nvar, ExcInh, P, randomvL, randomgL, randomgsd,indexAMPA,indexNMDA,indexGABA,indexsynAMPA,indexsynNMDA,indexsynGABA,pRelAMPA,pRelNMDA,pRelGABA,pRel_stfAMPA,pRel_stfNMDA,pRel_stfGABA)
    ...
    for postsyn_neuron=0:(nNeurons-1)
        ...
        for presyn_neuron=1:nNeurons
            fact_AMPA += gAMPA*sAMPA_vector[presyn_neuron]*P[postsyn_neuron+1,presyn_neuron]*pRelAMPA[presyn_neuron]*pRel_stfAMPA[presyn_neuron];
            fact_NMDA += gNMDA*sNMDA_vector[presyn_neuron]*P[postsyn_neuron+1,presyn_neuron]*pRelNMDA[presyn_neuron]*pRel_stfNMDA[presyn_neuron];
            fact_GABA += gGABA*synGABA_vector[presyn_neuron]*P[postsyn_neuron+1,presyn_neuron]*pRelGABA[presyn_neuron]*pRel_stfGABA[presyn_neuron];
        end
    
        if ExcInh[postsyn_neuron+1] == 0
            #pyramidal_neuron
            #soma's voltage
            vs = x[1+index];
            #dendrite's voltage
