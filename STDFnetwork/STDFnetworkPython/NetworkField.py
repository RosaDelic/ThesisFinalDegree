...
def NetworkField(t0, x, neq, nNeurons, nvar, ExcInh, P, randomvL, randomgL, randomgsd,indexAMPA,indexNMDA,indexGABA,indexsynAMPA,indexsynNMDA,indexsynGABA,pRelAMPA,pRelNMDA,pRelGABA,pRel_stfAMPA,pRel_stfNMDA,pRel_stfGABA):
    ...
    # network loop
    for postsyn_neuron in prange(nNeurons):
        ...
        neuron_type = ExcInh[postsyn_neuron]
        ...
        #fact_AMPA = np.sum(gAMPA*sAMPA_vector*P[postsyn_neuron,:]*pRelAMPA*pRel_stfAMPA)

        #iterate over all presyn neurons (matrix P by rows) for NMDA and sum all presynaptic neurons contributions
        #fact_NMDA = np.sum(gNMDA*sNMDA_vector*P[postsyn_neuron, :]*pRelNMDA*pRel_stfNMDA)

        #iterate over all presyn neurons (matrix P by rows) for GABA and sum all presynaptic neurons contributions    
        #fact_GABA = np.sum(gGABA*sGABA_vector*P[postsyn_neuron, :]*pRelGABA*pRel_stfGABA)
        ...
        if not ExcInh[postsyn_neuron]:
            ...
            mNapinf=1/(1+np.exp(-(vd+55.7)/7.7))
            ...
            #sodium (Na)
            am=0.5*(v+35)/(1-np.exp(-(v+35)/10))
            bm=20*np.exp(-(v+60)/18)
            minf=am/(am+bm)
            Ina=Interneuron_gna*(minf**3)*h*(v-Interneuron_vNa)
            ah=0.35*np.exp(-(v+58)/20)
            bh=5/(1+np.exp(-(v+28)/10))

            #delayed rectifier potassium (K)
            Ik=Interneuron_gk*(n**4)*(v-Interneuron_vK)
            an=0.05*(v+34)/(1-np.exp(-(v+34)/10))
            bn=0.625*np.exp(-(v+44)/80)
