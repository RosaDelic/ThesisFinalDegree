...
def Prerelease(x0,pre,nNeurons,neq,rk45_TOL,fD_AMPA,fD_NMDA,fD_GABA,fF_AMPA,fF_NMDA,fF_GABA,indexvspyramneuron,indexvinterneuron,ExcInh,pRelAMPA,pRelNMDA,pRelGABA,pRel_stfAMPA,pRel_stfNMDA,pRel_stfGABA):
    ...
    factor_relAMPA = np.exp(-rk45_TOL/tau_relAMPA)
    factor_relNMDA = np.exp(-rk45_TOL/tau_relNMDA)
    factor_relGABA = np.exp(-rk45_TOL/tau_relGABA)
    #Adding facilitation factors    
    factor_stfAMPA = np.exp(-rk45_TOL/tau_stfAMPA)
    factor_stfNMDA = np.exp(-rk45_TOL/tau_stfNMDA)
    factor_stfGABA = np.exp(-rk45_TOL/tau_stfGABA)
