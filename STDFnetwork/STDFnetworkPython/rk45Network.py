...
def rk45Network(RHS, t0, tf, x0, N, h, neq, nNeurons, nvar, P, ExcInh, fD_AMPA, fD_NMDA, fD_GABA, fF_AMPA, fF_NMDA, fF_GABA, randomvL, randomgL, randomgsd):
    ...
    #For depression
    pRelAMPA = np.ones((nNeurons,N+1))
    pRelNMDA = np.ones((nNeurons,N+1))
    pRelGABA = np.ones((nNeurons,N+1))
    #For facilitation
    pRel_stfAMPA = np.ones((nNeurons,N+1))
    pRel_stfNMDA = np.ones((nNeurons,N+1))
    pRel_stfGABA = np.ones((nNeurons,N+1))
